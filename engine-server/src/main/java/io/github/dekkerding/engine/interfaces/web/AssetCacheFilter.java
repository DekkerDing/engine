 ...
 * "public, max-age=31536000, immutable"（openspec single-jar-deployment
 * 「静态资源缓存头策略 · 哈希资产长缓存」）。
 *
...
public class AssetCacheFilter implements Filter {
     ...
     * <ul>
     *   <li>dispatcherTypes 含 FORWARD —— 资源链解析与内部转发派发的请求
     *       也要带上这层缓存头；</li>
     *   <li>最高优先级 —— 缓存头要在任何可能提交响应的组件之前就位。
     *       与 {@code ApiPrefixRewriteFilter} 的 URL 模式（/api/*）不相交，
     *       互不影响。</li>
     * </ul>
     */
    @Bean
