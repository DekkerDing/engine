 ...
 * （openspec single-jar-deployment「前端静态资源与 SPA 回退」）。
 *
 * <p>【问题】React Router 的路由只存在于浏览器内存里：用户在 /search 按
 * F5，浏览器向服务真的发 GET /search——classpath 里并没有这个文件，
 * 没有回退逻辑就是 404。
 *
...
public class SpaFallbackResolver extends PathResourceResolver {
    ...
    /** 前端入口页（构建产物位置：engine-server/frontend/dist/index.html → jar 内 static/） */
